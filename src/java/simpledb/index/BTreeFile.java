...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        BTreeInternalPage bTreePage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
        Iterator<BTreeEntry> entryIterator = bTreePage.iterator();
        BTreeEntry bTreeEntry = null;
        BTreePageId nextPageId = null;
        if (entryIterator.hasNext()) {
            bTreeEntry = entryIterator.next();
        } else {
        ...
        if (f == null) {
            ...
            while (f.compare(Op.GREATER_THAN, bTreeEntry.getKey()) && entryIterator.hasNext()) {
                bTreeEntry = entryIterator.next();
            }
            if (f.compare(Op.LESS_THAN_OR_EQ, bTreeEntry.getKey())) {
                nextPageId = bTreeEntry.getLeftChild();
            } else {
                nextPageId = bTreeEntry.getRightChild();
            }
        }
    ...
    /**
     * Split a leaf page to make room for new t uples and recursively split the parent node
...
//        create new page
        ...
        ;
        Tuple[] tuplesMove = new Tuple[(page.getNumTuples() + 1) / 2];
//        move the tuples
        int moveCnt = tuplesMove.length - 1;
        while (moveCnt >= 0 && iterator.hasNext()) {
            tuplesMove[moveCnt--] = iterator.next();
        }

        for (int i = tuplesMove.length - 1; i >= 0; i--) {
            page.deleteTuple(tuplesMove[i]);
            newPage.insertTuple(tuplesMove[i]);
        }
        Field midKey = tuplesMove[0].getField(keyField);
...
//        set up parent
        ...
        BTreeEntry parentEntry = new BTreeEntry(midKey, page.getId(), newPage.getId());
        ...
        if (oldRight != null) {
            ...
            dirtypages.put(oldRight, oldRightPage);
        }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry[] entryMove = new BTreeEntry[(page.getNumEntries() + 1) / 2];
        int cnt = entryMove.length - 1;
        while (cnt >= 0 && iterator.hasNext()) {
            entryMove[cnt--] = iterator.next();
        }
        BTreeEntry midKey = null;
//        remove entry
        for (int i = entryMove.length - 1; i >= 0; i--) {
            if (i == 0) {
                page.deleteKeyAndLeftChild(entryMove[i]);
                midKey = entryMove[0];
            } else {
                page.deleteKeyAndRightChild(entryMove[i]);
                newPage.insertEntry(entryMove[i]);
            }
            updateParentPointer(tid, dirtypages, newPage.getId(), entryMove[i].getRightChild());
        }
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midKey.getKey());
//        update child
        midKey.setLeftChild(page.getId());
        midKey.setRightChild(newPage.getId());
        parentPage.insertEntry(midKey);
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int stealCount = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        Tuple[] stealTuple = new Tuple[stealCount];
        int cnt = stealCount - 1;
        Tuple midkey = null;
        Iterator<Tuple> iterator = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
        while (cnt >= 0 && iterator.hasNext()) {
            stealTuple[cnt--] = iterator.next();
        }
        for (int i = stealCount - 1; i >= 0; i--) {
            sibling.deleteTuple(stealTuple[i]);
            page.insertTuple(stealTuple[i]);
            if (i == 0) {
                midkey = stealTuple[i];
            }
        }
        ...
        entry.setKey(midkey.getField(keyField));
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int stealCount = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
        BTreeEntry[] stealEntry = new BTreeEntry[stealCount];
        int cnt = stealCount - 1;
        while (cnt >= 0 && iterator.hasNext()) {
            stealEntry[cnt--] = iterator.next();
        }
        for (int i = stealCount - 1; i >= 0; i--) {
            BTreeEntry temp = stealEntry[i];
            leftSibling.deleteKeyAndRightChild(temp);
            updateParentPointer(tid, dirtypages, page.getId(), temp.getRightChild());
            BTreePageId pageId = temp.getRightChild();
// updata parent
            temp.setLeftChild(parentEntry.getLeftChild());
            temp.setRightChild(parentEntry.getRightChild());
            temp.setRecordId(parentEntry.getRecordId());
            parent.updateEntry(temp);
//update this page
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.
        int stealCount = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> iterator = rightSibling.iterator();

        for (int i = 0; i < stealCount; i++) {
            if (!iterator.hasNext()) {
                throw new DbException("no new entry");
            }
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if (right != null) {
            BTreeLeafPage temp = (BTreeLeafPage) getPage(tid,dirtypages,right,Permissions.READ_WRITE);
            temp.setLeftSiblingId(leftPage.getId());
            dirtypages.put(right,temp);
        }
...
//        BTreePageId temp = iterator.next().getLeftChild();
//        parentEntry.setRightChild(temp);
